...
import { Clock, CheckCircle, Circle, Calendar, Users, MapPin, AlertCircle, Menu, Home, BarChart3, Filter, Bell, X, Settings, TrendingUp, Award, Target, Zap, FileText, Check, Pill, Gift, Star, Coffee, Car, Plane, Wifi, WifiOff, Download, Smartphone } from 'lucide-react';
...
const KrankenhausLogistikApp = () => {
  ...
  // ✅ YENİ: DECT Kilidi Sistemi - Günde sadece bir DECT'ten puan alma
  ...
  const syncOfflineChanges = async () => {
    ...
    try {
      ...
      addNotification('❌ Synchronisation fehlgeschlagen. Versuche später erneut.', 'error');
    }
  };
  ...
  useEffect(() => {
    const loadStoredData = () => {
      ...
      if (lastSavedDate !== today) {
        ...
        // ✅ ALMANCA BİLDİRİM
        addNotification('🌅 Neuer Arbeitstag begonnen! Alle Aufgaben wurden zurückgesetzt.', 'info');
      } else {
  ...
  const toggleTask = (taskId) => {
    ...
    if (newCompletedTasks.has(taskId)) {
      ...
      // Görev tamamlanırsa ve puan alma hakkı varsa puan kazan
      if (canEarnPoints) {
  ...
  useEffect(() => {
    const updateStats = () => {
      ...
      setWeeklyTrends(prev => 
        prev.map((week, index) => 
          index === prev.length - 1 
            ? { ...week, completion: rate, efficiency: Math.min(rate + 5, 100), quality: Math.min(rate + 3, 100) }
            : week
        )
      );
    };
  ...
  // Belohnung Sistemi
  const rewards = [
    { id: 'coffee', name: 'Kostenloser Kaffee', icon: '☕', points: 350, description: 'Gratis Kaffee aus dem Automaten' },
    { id: 'lunch', name: 'Kostenloses Mittagessen', icon: '🍽️', points: 750, description: 'Gratis Menü in der Kantine' },
    { id: 'parking', name: '1 Tag Kostenlose Parkplatz', icon: '🅿️', points: 750, description: '1 Tag VIP Parkplatz näher zum Eingang' },
    { id: 'wellness', name: 'Wellness Gutschein', icon: '💆', points: 2250, description: '50€ Gutschein für Spa/Massage' }
  ];
  ...
  const getAllDepartmentStats = () => {
    return Object.keys(departments).map(deptCode => {
      ...
      return {
        ...
        status: rate >= 80 ? 'excellent' : rate >= 60 ? 'good' : 'needs_attention'
      };
    });
  };
          ...
          {showSettings && (
            <div className="mt-4 p-3 sm:p-4 bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/30 max-w-4xl mx-auto">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-gray-900 text-sm sm:text-base">Erinnerungseinstellungen</h3>
          ...
          {showLeaderDashboard && isLeaderMode && (
            <div className="mt-4 p-3 sm:p-4 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/30 max-h-96 overflow-y-auto max-w-6xl mx-auto">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={handleLeaderLogout} className="p-1 rounded-lg hover:bg-gray-100">
                  <X className="w-4 h-4 text-gray-500" />
                </button>
              </div>
              ...
              <div className="mb-6 p-3 sm:p-4 bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl border border-purple-200">
                <h4 className="font-bold text-purple-800 mb-3 text-sm sm:text-base">📊 Gesamtübersicht</h4>
                <div className="grid grid-cols-2 gap-2 sm:gap-4">
                  <div className="text-center">
                    <div className="text-xl sm:text-2xl font-bold text-purple-700">
                      {getAllDepartmentStats().reduce((sum, dept) => sum + dept.completed, 0)}
                    </div>
                    <div className="text-xs text-purple-600">Erledigte Aufgaben</div>
                  </div>
                  <div className="text-center">
                    <div className="text-xl sm:text-2xl font-bold text-purple-700">
                      {Math.round(getAllDepartmentStats().reduce((sum, dept) => sum + dept.rate, 0) / getAllDepartmentStats().length)}%
                    </div>
                  ...
                  {getAllDepartmentStats().map((dept) => (
                    <div
                      ...
                      className={`p-3 rounded-xl border transition-all ${
                        dept.status === 'excellent' ? 'bg-green-50 border-green-300' :
                        dept.status === 'good' ? 'bg-yellow-50 border-yellow-300' :
                      ...
                      <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="font-bold text-sm text-gray-800 flex items-center">
                            ...
                            <button
                              onClick={(e) => {
                                ...
                                console.log('Button clicked for dept:', dept.code);
                        ...
                        <div className="text-right">
                          <div className={`text-lg font-bold ${
                            ...
                            dept.status === 'good' ? 'text-yellow-700' :
                            'text-red-700'
                          }`}>
                            {dept.rate}%
                          </div>
                          <div className="text-xs text-gray-600">{dept.completed}/{dept.total}</div>
                        </div>
                      </div>
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div
                          className={`h-2 rounded-full transition-all duration-500 ${
                            dept.status === 'excellent' ? 'bg-gradient-to-r from-green-400 to-green-500' :
                            dept.status === 'good' ? 'bg-gradient-to-r from-yellow-400 to-yellow-500' :
                            'bg-gradient-to-r from-red-400 to-red-500'
                          }`}
              ...
              <div className="mt-4 p-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200">
                <div className="text-xs text-blue-600 text-center">
                  <strong>Letztes Update:</strong> {new Date().toLocaleString('de-DE')}
                </div>
              </div>
            </div>
          )}
            ...
            <div className="mt-4 p-3 sm:p-4 bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/30 max-h-96 overflow-y-auto max-w-6xl mx-auto">
                ...
                <div className="bg-gradient-to-br from-blue-50 to-cyan-100 p-3 rounded-xl border border-blue-200">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full ${
                      departmentMetrics.avgCompletionTime.trend === 'up' ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'
                    }`}>
                      {departmentMetrics.avgCompletionTime.trend === 'up' ? '↗️ +8%' : '↘️ -5%'}
                    </span>
                  </div>
                  <div className="text-base sm:text-lg font-bold text-blue-700">{departmentMetrics.avgCompletionTime.current}min</div>
                  <div className="text-xs text-blue-600">Ø Aufgabenzeit</div>
                  <div className="text-xs text-gray-500 mt-1">Vorher: {departmentMetrics.avgCompletionTime.previous}min</div>
                </div>
                
                <div className="bg-gradient-to-br from-green-50 to-emerald-100 p-3 rounded-xl border border-green-200">
                  <div className="flex items-center justify-between mb-2">
                    <Award className="w-4 h-4 text-green-600" />
                    <span className="text-xs px-2 py-1 rounded-full bg-green-200 text-green-800">↗️ +3%</span>
                  </div>
                  <div className="text-base sm:text-lg font-bold text-green-700">{departmentMetrics.qualityScore.current}%</div>
                  <div className="text-xs text-green-600">Qualitätsscore</div>
                  <div className="text-xs text-gray-500 mt-1">Vorher: {departmentMetrics.qualityScore.previous}%</div>
                </div>
                
                <div className="bg-gradient-to-br from-purple-50 to-pink-100 p-3 rounded-xl border border-purple-200">
                  <div className="flex items-center justify-between mb-2">
                    <Target className="w-4 h-4 text-purple-600" />
                    <span className="text-xs px-2 py-1 rounded-full bg-green-200 text-green-800">↗️ +4%</span>
                  </div>
                  <div className="text-base sm:text-lg font-bold text-purple-700">{departmentMetrics.onTimeRate.current}%</div>
                  <div className="text-xs text-purple-600">Pünktlichkeit</div>
                  <div className="text-xs text-gray-500 mt-1">Vorher: {departmentMetrics.onTimeRate.previous}%</div>
                </div>
                
                <div className="bg-gradient-to-br from-orange-50 to-yellow-100 p-3 rounded-xl border border-orange-200">
                  <div className="flex items-center justify-between mb-2">
                    <Zap className="w-4 h-4 text-orange-600" />
                    <span className="text-xs px-2 py-1 rounded-full bg-green-200 text-green-800">↗️ -20%</span>
                  </div>
                  <div className="text-base sm:text-lg font-bold text-orange-700">{departmentMetrics.taskVariation.current}min</div>
                  <div className="text-xs text-orange-600">Zeitabweichung</div>
                  <div className="text-xs text-gray-500 mt-1">Vorher: {departmentMetrics.taskVariation.previous}min</div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="bg-gray-50 rounded-xl p-3 sm:p-4">
                        ...
                        <div className="flex flex-col items-center space-y-1 mb-3">
                          ...
                          <div className="w-4 sm:w-6 bg-gray-200 rounded-t-lg relative" style={{height: '100px'}}>
                            <div
                              ...
                              style={{height: `${(week.efficiency / 100) * 100}px`}}
                            ></div>
                          </div>
                          {/* Quality */}
                          <div className="w-4 sm:w-6 bg-gray-200 rounded-t-lg relative" style={{height: '100px'}}>
                            <div
                              ...
                              style={{height: `${(week.quality / 100) * 100}px`}}
                            ></div>
                          </div>
                        </div>
                        <span className="text-xs sm:text-sm text-gray-700 font-bold bg-white px-2 py-1 rounded-lg shadow-sm border">
                          {week.week}
                        </span>
                      </div>
                    ))}
                  ...
                  {/* ✅ IMPROVED LEGEND */}
                  <div className="flex justify-center space-x-3 sm:space-x-6 mt-4 p-3 bg-white rounded-lg border border-gray-200">
                    <div className="flex items-center">
                      <div className="w-4 h-4 bg-gradient-to-r from-blue-500 to-cyan-400 rounded mr-2"></div>
                      <span className="text-xs sm:text-sm font-medium text-gray-700">Abschluss</span>
                    </div>
                    <div className="flex items-center">
                      <div className="w-4 h-4 bg-gradient-to-r from-green-500 to-emerald-400 rounded mr-2"></div>
                      <span className="text-xs sm:text-sm font-medium text-gray-700">Effizienz</span>
                    </div>
                    <div className="flex items-center">
                      <div className="w-4 h-4 bg-gradient-to-r from-purple-500 to-pink-400 rounded mr-2"></div>
                      <span className="text-xs sm:text-sm font-medium text-gray-700">Qualität</span>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 text-center">
                    <div className="text-xs sm:text-sm text-gray-600 space-y-1">
                      <div className="font-bold text-gray-800">Aktuelle Woche (KW 5):</div>
                      <div className="flex justify-center space-x-4 text-xs">
                        <span className="text-blue-600">📊 {weeklyTrends[4].completion}% Abschluss</span>
                        <span className="text-green-600">⚡ {weeklyTrends[4].efficiency}% Effizienz</span>
                        <span className="text-purple-600">⭐ {weeklyTrends[4].quality}% Qualität</span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div>
                ...
                <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-3 sm:p-4">
                  <div className="flex items-end justify-between h-24 sm:h-32 mb-4">
                    {hourlyActivity.map((hour, index) => (
                      <div key={index} className="flex flex-col items-center">
                        <div className="relative">
                          <div className="w-5 sm:w-8 bg-gray-200 rounded-t-lg relative" style={{height: '80px'}}>
                            <div
                        ...
                        <span className="text-xs sm:text-sm text-gray-700 mt-2 font-medium bg-white px-1 sm:px-2 py-1 rounded-lg shadow-sm border">
                  ...
                  <div className="bg-white rounded-lg p-3 border border-indigo-200">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs sm:text-sm">
                      <div className="text-center">
                        <div className="font-bold text-indigo-700">🔥 Peak Zeit</div>
                        <div className="text-indigo-600">08:00-09:00 Uhr</div>
                        <div className="text-xs text-gray-500">4 Aufgaben/Stunde</div>
                      </div>
                      <div className="text-center">
                        <div className="font-bold text-green-700">😌 Ruhigste Zeit</div>
                        <div className="text-green-600">12:00-13:00 Uhr</div>
              ...
              <div className="mb-6 p-3 sm:p-4 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl border border-yellow-200">
                ...
                <div className="w-full bg-yellow-200 rounded-full h-2">
                  <div
                    ...
                    style={{width: `${Math.min((userPoints / 2250) * 100, 100)}%`}}
                  ></div>
                </div>
              </div>
            ...
            <div className="mt-4 p-3 sm:p-4 bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/30 max-w-2xl mx-auto">
              <div className="space-y-4">
                      ...
                      <div className="p-2">
                        ...
                        <div className="py-2">
                          {Object.entries(departments).map(([code, name]) => (
                            ...
                            >
                              <div className="flex items-center space-x-3">
                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-white font-bold text-xs shadow-lg ${
                                  selectedDepartment === code 
                ...
                <div className="lg:hidden grid grid-cols-2 gap-2 pt-2 border-t border-gray-200">
                  <button
                    onClick={() => {setShowDocumentation(!showDocumentation); setShowMenu(false);}}
                  ...
                  <button
                    onClick={() => {setShowApotheke(!showApotheke); setShowMenu(false);}}
                  ...
                  <button
                    onClick={() => {setShowRewards(!showRewards); setShowMenu(false);}}
      ...
      {completedCount === currentTasks.length && currentTasks.length > 0 && (
        <div className="fixed bottom-4 sm:bottom-6 left-3 right-3 sm:left-4 sm:right-4 max-w-lg mx-auto z-50">
          <div className="bg-gradient-to-r from-green-500 via-emerald-500 to-teal-500 text-white p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-2xl text-center border border-white/20 backdrop-blur-xl">
            <div className="text-xl sm:text-2xl font-bold mb-2">🎉 Alle Aufgaben erledigt! 🎉</div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-4">
          <div className="bg-white/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/30 w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white/90 backdrop-blur-xl p-3 sm:p-4 border-b border-gray-200 flex items-center justify-between">
              ...
              <button onClick={() => setShowStatistics(false)} className="p-1 rounded-lg hover:bg-gray-100">
                <X className="w-4 h-4 text-gray-500" />
              </button>
            </div>
            ...
            <div className="p-3 sm:p-4">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-6">
                <div className="bg-gradient-to-br from-blue-50 to-indigo-100 p-3 rounded-xl border border-blue-200">
                  ...
                  <div className="text-lg sm:text-xl font-bold text-blue-700">{dailyStats.completionRate}%</div>
                ...
                <div className="bg-gradient-to-br from-green-50 to-emerald-100 p-3 rounded-xl border border-green-200">
                  ...
                  <div className="text-lg sm:text-xl font-bold text-green-700">{dailyStats.completedTasks}/{dailyStats.totalTasks}</div>
                ...
                <div className="bg-gradient-to-br from-purple-50 to-pink-100 p-3 rounded-xl border border-purple-200">
                  ...
                  <div className="text-lg sm:text-xl font-bold text-purple-700">{dailyStats.averageTimePerTask}min</div>
                ...
                <div className="bg-gradient-to-br from-orange-50 to-yellow-100 p-3 rounded-xl border border-orange-200">
                  ...
                  <div className="text-lg sm:text-xl font-bold text-orange-700">{dailyStats.streakDays}</div>
                ...
                <div className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl p-3 sm:p-4 border border-gray-200">
                  <div className="flex items-end justify-between h-32 sm:h-40 mb-4">
                    {weeklyData.map((day, index) => {
                      ...
                      const isToday = index === 4; // Freitag ist heute
                      return (
                        <div key={index} className="flex flex-col items-center flex-1">
                          <div className="relative">
                            <div className={`w-6 sm:w-10 bg-gray-200 rounded-t-lg relative ${isToday ? 'ring-2 ring-blue-400' : ''}`} style={{height: '100px'}}>
                              <div
                                className={`${isToday ? 'bg-gradient-to-t from-blue-500 to-indigo-500' : 'bg-gradient-to-t from-indigo-400 to-purple-400'} rounded-t-lg w-full absolute bottom-0 transition-all duration-500`}
                                style={{height: `${Math.max(height, 5)}px`}}
                              ></div>
                              {/* ✅ Completion display */}
                              {day.total > 0 && (
                                <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 text-xs font-bold text-gray-700 bg-white px-1 rounded shadow">
                                  {day.completed}/{day.total}
                                </div>
                              )}
                            </div>
                          </div>
                          <span className={`text-xs sm:text-sm font-medium mt-2 px-2 py-1 rounded-lg ${
                            isToday ? 'bg-blue-500 text-white shadow-lg' : 'bg-white text-gray-700 border'
                          }`}>
                            {day.day}
                          </span>
                        </div>
                  ...
                  <div className="bg-white rounded-lg p-3 border border-gray-200">
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center text-xs">
                      <div>
                        <div className="font-bold text-green-700">Wochensumme</div>
                        <div className="text-green-600">{weeklyData.reduce((sum, day) => sum + day.completed, 0)} Aufgaben</div>
                      </div>
                      <div>
                        <div className="font-bold text-blue-700">Beste Tag</div>
                        <div className="text-blue-600">Mittwoch (100%)</div>
                      </div>
                      <div>
                        <div className="font-bold text-orange-700">Ø Leistung</div>
                        <div className="text-orange-600">{Math.round(weeklyData.reduce((sum, day) => sum + (day.total > 0 ? (day.completed/day.total)*100 : 0), 0) / weeklyData.filter(d => d.total > 0).length)}%</div>
                      </div>
                      <div>
                        <div className="font-bold text-purple-700">Trend</div>
                        <div className="text-purple-600">📈 Steigend</div>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  {updatedDepartmentPerformance.map((dept, index) => (
                    ...
                    >
                      <span className={`text-sm font-medium truncate flex-1 ${
                        dept.dept === selectedDepartment ? 'text-blue-800' : 'text-gray-700'
                      }`}>
                        {dept.dept} {dept.dept === selectedDepartment && '(Sie)'}
                      </span>
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`text-sm font-bold ${
                          dept.dept === selectedDepartment ? 'text-blue-700' : 'text-gray-600'
                        }`}>
                          {dept.rate}%
                        </span>
                      </div>
              ...
              <div className="bg-gradient-to-r from-yellow-50 to-orange-50 p-3 sm:p-4 rounded-xl border border-yellow-200">
                ...
                <div className="flex flex-wrap gap-2">
                  {dailyStats.completionRate >= 80 && (
                    <div className="bg-yellow-400 text-yellow-900 px-2 py-1 rounded-full text-xs font-bold">🏆 Effizienz</div>
                  )}
                  {dailyStats.streakDays >= 3 && (
                    <div className="bg-blue-400 text-blue-900 px-2 py-1 rounded-full text-xs font-bold">🔥 Konsistenz</div>
                  )}
                  {dailyStats.completedTasks >= 8 && (
                    <div className="bg-green-400 text-green-900 px-2 py-1 rounded-full text-xs font-bold">⚡ Produktivität</div>
                  )}
                  {dailyStats.completionRate === 100 && (
                    <div className="bg-purple-400 text-purple-900 px-2 py-1 rounded-full text-xs font-bold">⭐ Perfektion</div>
                  )}
                </div>
              </div>
            </div>
